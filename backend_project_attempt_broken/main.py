...
def create_instance(instance: InstanceCreate, db: Session = Depends(get_db)):
    # 1. Validate User and Project Hierarchy
    ...
    if user.project_id != project.id and user.role != models.UserRoleEnum.SUPERADMIN:
        raise HTTPException(status_code=403, detail="User does not belong to this project")
    ...
    # 2. Enforce Project-Level GPU Quota
    current_project_instances = db.query(models.Instance).filter(
    ...
    current_gpu_usage = sum(
        db.query(models.RentalPlan).filter(models.RentalPlan.id == inst.plan_id).first().resource_count 
        for inst in current_project_instances
    )
    
    if (current_gpu_usage + requested_count) > project.max_gpu_count:
        raise HTTPException(
            status_code=409, 
            detail=f"Project quota exceeded. Max: {project.max_gpu_count}, Current: {current_gpu_usage}, Requested: {requested_count}"
        )

    # 3. K8s Provisioning Logic (Keep your existing Kubernetes V1Pod spec logic here)
    ...
    try:
        ...
        if usage["free"] < requested_count:
            raise HTTPException(status_code=409, detail="No available hardware capacity on node")

        short_uuid = uuid.uuid4().hex[:6]
        pod_name = f"gpu-p{project.id}-u{user.id}-{short_uuid}"

        # ... [Insert your existing client.V1Pod creation logic here] ...
        
        v1.create_namespaced_pod(namespace=namespace, body=pod_manifest)
        logger.info(f"Created pod {pod_name} for Project {project.id}")
...
# =========================
# Projects (Enterprise Multi-Tenancy)
# =========================
...
def create_project(project: ProjectCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Project creation failed. Name might already exist.")
